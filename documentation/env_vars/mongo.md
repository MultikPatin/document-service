...
- **PoolSettings**: Управляет параметрами пула подключений (размер, таймауты, мониторинг)
...
- **RepresentationSettings**: Контролирует представление данных (особенно формат UUID)
...
Логирование использует систему ведения журнала приложения с выделенным именем регистратора, определенным в
`LoggerNames.init()`.
